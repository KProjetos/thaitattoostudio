...
function Header() {
  ...
  return (
    <header className="w-full h-[85vh]  direction-col-center justify-center bg-principal z-50">
      <div className=" w-full min-h-[85vh] absolute top-0 bg-black/80 z-50"></div>
      <video
        className="w-full h-[85vh] mobile:absolute mobile:top-0 md:hidden object-fill z-0"
        autoPlay
        muted
        loop
        playsInline
      ...
      <video
        className="hidden w-full h-[85vh] object-fill md:block  md:absolute md:top-0 md:z-0"
        autoPlay
        muted
        loop
        playsInline
      ...
      <div className="w-[90%] max-w-[1080px]  text-second direction-col items-center lg:items-center justify-start z-50 font-aref">
        ...
        <p className="text-center mb-[2em]">
          Faça seu projeto exclusivo agora mesmo e visualize a arte no seu corpo
          através do Tablet, antes de tatuar.
        </p>

        <a
          target="_blank"
          href="https://wa.me/5511940767201"
          className="direction-col-center lg:text-[1.2em] p-0"
